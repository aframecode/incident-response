    ...
    <v-dialog v-model="overlay" max-width="600">
      <v-card v-if="incident">
        <v-card-title class="headline"
          ...
          {{ incident.description.subtype }}</v-card-title
        >
        ...
        >
          ...
          <v-tab-item v-for="i in tabs" :key="i" :value="i">
            <v-card flat tile>
              <v-card-text>
                <v-textarea
                  outlined
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="green darken-1" text @click="overlay = false">
            close
          </v-btn>
        </v-card-actions>
      </v-card>
  ...
  computed: {
    incidentLatLon: function() {
      if (this.incident !== undefined) {
        return [
          this.incident.address.latitude,
